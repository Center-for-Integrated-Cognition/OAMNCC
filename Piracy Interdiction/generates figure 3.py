...
NONINTERVENTION_PIRATE_SUCCESS_PROBABILITY_WATER_CANNONS = 0.95
PROB_PIRATE_SUCCESS_PER_MINUTE_WATER_CANNONS = 1.0 - (NONINTERVENTION_PIRATE_SUCCESS_PROBABILITY_WATER_CANNONS ** (1.0 / PIRATE_GIVE_UP_TIME_MINUTES))
...
def simulate_engagement_outcome(merchant_coords, merchant_ransom_value, naval_ship_intervention_coords, water_cannons):
    """
    ...
    prob_pirate_success_used = PROB_PIRATE_SUCCESS_PER_MINUTE
    if water_cannons:
        prob_pirate_success_used = PROB_PIRATE_SUCCESS_PER_MINUTE_WATER_CANNONS
    is_boarded = False
    for minute in range(int(math.ceil(duration_of_boarding_attempt))):
        if random.random() < prob_pirate_success_used:
            is_boarded = True
            break
...
def calculate_s3_estimated_marginal_gain(merchant_coords, naval_ship_start_coords, knows_about_water_cannons):
    """
    ...
    if est_t_hub_arrival < est_t_pirate_arrival:
        ...
        prob_pirate_fails_per_minute = 1.0 - prob_pirate_success_used
...
def execute_scenario(scenario_params,water_cannon_knowledge):
    ...
    for _ in range(num_trials):
        ...
        for name in TARGET_NAMES:
            ...
            trial_targets_data_for_strategies.append({
                ...
                "s3_marginal_gain_prob": calculate_s3_estimated_marginal_gain(coords, NAVAL_SHIP_START_COORDS, water_cannons if water_cannon_knowledge else False) #either water_cannons or just False, depending on condition. ## changeme
            })
        ...
        for target in trial_targets_data_for_strategies:
            nonintervention_prob = NONINTERVENTION_PIRATE_SUCCESS_PROBABILITY
            if target['water_cannons']:
                nonintervention_prob = NONINTERVENTION_PIRATE_SUCCESS_PROBABILITY_WATER_CANNONS
            trial_baseline_outcome += nonintervention_prob * target['ransom_value']#simulate_engagement_outcome(
...
#            )
        ...
        for strat_idx, chosen_target in enumerate([s1_choice, s2_choice, s3_choice]):
            ...
            current_trial_total_outcome += simulate_engagement_outcome(
                chosen_target['coords'], chosen_target['ransom_value'], chosen_target['coords'], chosen_target['water_cannons']
            )
            for target in trial_targets_data_for_strategies:
                if target['name'] != chosen_target['name']:
                    ...
                    if target['water_cannons']:
                        nonintervention_prob = NONINTERVENTION_PIRATE_SUCCESS_PROBABILITY_WATER_CANNONS
                    current_trial_total_outcome += nonintervention_prob * target['ransom_value'] #simulate_engagement_outcome(
#                        target['coords'], target['ransom_value'], (float('inf'), float('inf'))
#                    )
...
def plot_scenario_outcomes(desc, num_trials, s1_data, s2_data, s3_data, s4_data, 
    ...
    if s4_data and len(s4_data) > 1: data_to_plot.append(s4_data); labels.append('Marginal\nGain, Knows\nWater Cannons')
    else: s4_data = None
    ...
    ax.axvline(2.5, color='black', linestyle='--', linewidth=1, alpha=0.8)

    violin_colors = ['skyblue', 'lightgreen', 'lightcoral', 'lightcoral']
    ...
    s1_idx, s2_idx, s3_idx, s4_idx = -1, -1, -1, -1
...
#            if s3_idx != -1: 
#                y_pos_s2s3 = y_pos_s1s2 + line_height_offset * 2.5 + text_v_offset*2 
#                y_pos_s1s3 = y_pos_s2s3 + line_height_offset * 2.5 + text_v_offset*2
...
def main():
    ...
    results = execute_scenario(scenario_to_run,False)
    results_2 = execute_scenario(scenario_to_run,True)
    ...
    if results["num_trials"] > 0 :
        ...
        s4_relative = [r - b for r, b in zip(results_2["s3_raw_outcomes"], baseline_outcomes)]
       
        plot_scenario_outcomes(results["description"], results["num_trials"],
                               s1_relative, s2_relative, s3_relative, s4_relative, 
                               plot_title_suffix=" (Relative to No Intervention)",
                               y_axis_label='Ransom Avoided',
                               filename_prefix="watercannon2_")
    else:
